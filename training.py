...
from model_periodicBC import P_network
from utils_periodicBC import *


batch_size = 32
num_training_samples = 4*16384
...
with tf.device("gpu:0"):
    lr_ = np.random.uniform(-5, -4)
    lr = tfe.Variable(10**lr_)
...
def loss(model, n, A_stencil, A_matrices, S_matrices,index = None,pos=-1.,phase="Training",epoch=-1,grid_size=8,remove=True):
    with tf.device("gpu:0"):
        ...
        # theta_y = np.array([i * 2 * pi / n for i in range(-n // 8 + 1, n // 8 + 1)])
    # P_stencil = bilinear_stencil(A_matrices.shape[0])
    with tf.device("gpu:0"):
        if phase == "Test" and epoch == 0:
            ...
            with tf.device('cpu:0'):
                iteration_matrix_numpy = iteration_matrix.numpy()
                eigs = np.linalg.eigvals(iteration_matrix_numpy)
                spectral_radius = np.mean(np.max(np.max(np.absolute(eigs), axis=2), axis=1))
            print("Black box - spcetral radius: ", spectral_radius)
            print("Black box - frobenious loss: ", loss_test)
        ...
        if not (phase == "Test" and epoch == 0):
            ...
            A_c = tf.reshape(A_c,(-1,int(theta_x.shape[0])**2,(grid_size//2)**2,(grid_size//2)**2))
            A_c_removed = tf.concat([A_c[:,:index_to_remove],A_c[:,index_to_remove+1:]],1)
            P_matrix_t_reshape = tf.reshape(P_matrix_t,(-1,int(theta_x.shape[0])**2,(grid_size//2)**2,grid_size**2))
            P_matrix_reshape = tf.reshape(P_matrix,
                                            (-1, int(theta_x.shape[0]) ** 2, grid_size ** 2, (grid_size//2) ** 2))
            A_matrices_reshaped = tf.reshape(A_matrices,
                                            (-1, int(theta_x.shape[0]) ** 2, grid_size ** 2, grid_size ** 2))
            ...
            if remove:
                ...
                # loss = tf.reduce_mean(
                #     tf.reduce_mean(tf.pow(tf.reduce_sum(tf.square(tf.abs(iteration_matrix)), [2, 3]), 1), 1))
            else:
                # loss = tf.reduce_mean(
                #     tf.reduce_max(tf.reduce_sum(tf.square(tf.abs(iteration_matrix)), [2, 3]), 1))
                loss = tf.reduce_mean(
                    tf.reduce_mean(tf.reduce_sum(tf.square(tf.abs(iteration_matrix_all)), [2, 3]), 1))

                print("Real loss: ", loss.numpy())
            print("smooth fro: ", tf.reduce_mean(tf.reduce_sum(tf.square(tf.abs(iteration_matrix_all[:,index_to_remove])), [1, 2])))
            if phase == "Test":
                print("Net loss - loss: ", loss.numpy())
...
m = P_network(grid_size=grid_size)
...
A_stencils_test, A_matrices_test, S_matrices_test,num_of_modes = get_A_S_matrices(num_test_samples,pi,grid_size,n_test)
...
A_stencils_train = np.array(two_d_stencil(num_training_samples))
n_train_list = [16,16,16,64,128,256]
initial_epsi = 1e-0
epsi_list = [initial_epsi/1.,initial_epsi/4.,initial_epsi/16.,initial_epsi/64.]
epsi_list = [0.9,0.4,0.1,0.05]
...
for j in range(len(n_train_list)):
    ...
    for epoch in range(2):
        ...
        _ = grad(m, n_test, A_stencils_test, A_matrices_test, S_matrices_test,phase="Test",epoch=epoch,grid_size=grid_size)
        if epoch % 1 == 0: #change to save once every X epochs
            root.save(file_prefix=checkpoint_perfix)

        training_losses = []
        for iter in range(num_training_samples//batch_size):
            idx = np.random.choice(A_stencils.shape[0], batch_size, replace=False)
            A_matrices = np.stack([[compute_A(A_stencils[idx], tx, ty, 1j,
                                              grid_size=grid_size) for tx in theta_x]
                                   for ty in theta_y])
            A_matrices = A_matrices.transpose((2, 0, 1, 3, 4))

            S_matrices = np.reshape(compute_S(A_matrices.reshape((-1, grid_size ** 2, grid_size ** 2))),
            ...
            grad(m, n_train, A_stencils_tensor, A_matrices_tensor, S_matrices_tensor,epoch=1,
            ...
            training_losses.append(real_loss)
    ...
    temp = create_coarse_training_set(m, pi,num_training_samples)
    A_stencils_train = np.concatenate([np.array(two_d_stencil(num_training_samples)), temp], axis=0)
