    ...
    def call(self, inputs,black_box=False,index=None,pos=-1.,phase='Training'):
        with tf.device("gpu:0"):
            ...
            right_contributions_input = tf.gather(inputs,
                                                  [i for i in range(2,self.grid_size,2)], axis=1)
            ...
            up_contributions_input = tf.gather(inputs, [i for i in range(1,self.grid_size,2)], axis=1)
            ...
            center_contributions_input = tf.gather(inputs,
                                                   [i for i in range(1, self.grid_size, 2)], axis=1)
            center_contributions_input = tf.gather(center_contributions_input, [i for i in range(1, self.grid_size, 2)],
                                                   axis=2)
            ...
            inputs_combined_temp_fixed = tf.where(tf.equal(inputs_combined_temp,0),1e20*tf.ones_like(inputs_combined_temp),inputs_combined_temp)
            ...
            if black_box:
                ...
                right_contributions_output = tf.gather(right_contributions_output, [i for i in range(1,self.grid_size,2)], axis=2)
                ...
                right_contributions_output = tf.concat([right_contributions_output,right_contributions_output_bb],axis=1)
                ...
                down_contributions_output = tf.concat([down_contributions_output_bb,down_contributions_output],axis=-1)
